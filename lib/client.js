...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* Font sizes reference the dsh web theme's font-scale tokens
		   (--dsw-font-*-font-size, defined on body by dsh-client-ui-theme), with
		   px fallbacks outside the web shell. Colors and code family already use
		   the --dsw-alias-* / --ds-font-family-* tokens, so every type style now
		   tracks the UI layer instead of being hard-coded. */
		var css = [
			...
			'.dsb-refresh{flex:none;width:22px;height:22px;border:0;background:transparent;color:var(--dsw-alias-label-secondary,#888);cursor:pointer;border-radius:50%;font-size:var(--dsw-font-xs-13-font-size,13px);line-height:1}',
			...
			'.dsb-label{font-size:var(--dsw-font-xxxs-11-font-size,11px);color:var(--dsw-alias-label-secondary,#888)}',
			'.dsb-num{font-size:var(--dsw-font-xxs-12-font-size,12px);font-variant-numeric:tabular-nums;font-family:var(--ds-font-family-code,monospace)}',
			'.dsb-rail{font-size:var(--dsw-font-xxxs-11-font-size,11px);font-family:var(--ds-font-family-code,monospace);font-variant-numeric:tabular-nums;line-height:1;padding:2px 0}',
