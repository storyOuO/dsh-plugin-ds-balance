 ...
 *
 * The daily "today" counters roll over at the host machine's LOCAL midnight
 * (the finished day is archived into `history`); peak pricing is independent
 * and stays on Beijing time.
 *
...
/**
 * Local calendar date 'YYYY-MM-DD' from the host machine's clock — the basis
 * of the daily rollover. `toISOString()` would yield the UTC date, which lags
 * or leads the local day by the timezone offset (up to 8 h for UTC+8), so the
 * reset must come from the local getters.
 * @param nowMs - epoch milliseconds; defaults to the current time.
 */
function localDayString(nowMs = Date.now()) {
 ...
 * archiving the finished day's cost into `history`. Returns whether the day
 * actually rolled, so callers can persist the change right away.
 * @param nowMs - epoch milliseconds; defaults to the current time.
 */
function rollDay(state, nowMs = Date.now()) {
  const today = localDayString(nowMs)
  if (state.day !== today) {
...
async function refreshBalance(env, state, file) {
  // Roll the day on the poll cadence too: with no LLM calls after local
  // midnight, the reset must still happen (and persist) on its own.
  rollDay(state)
...
function apply(ctx, config) {
  ...
  ctx.webServer.register({
    ...
    handler: async (_req, res) => {
      // Keep the snapshot on the LOCAL day between usage records: a midnight
      // rollover must reach the sidebar on its own, not wait for the next
      // LLM call. Persist immediately when the day actually rolled.
      if (rollDay(state)) saveState(file, state)
      res.writeHead(200, {
